...
export const command = new SlashCommandBuilder()
    ...
    .addNumberOption(option => {
        return option.setName('brightness')
            .setDescription('The brightness of the image, where 1 is white and 0 is black')
            .setMinValue(0)
            .setMaxValue(1)
    })
    .addNumberOption(option => {
        return option.setName('scale')
...
export const run: CommandRun = async (c, interaction) => {
    const { value: code } = interaction.options.get('code', true) as { value: string };
    const { value: brightness = 0.8 } = (interaction.options.get('brightness') || {}) as { value?: number };
    const { value: scale = 2 } = (interaction.options.get('scale') || {}) as { value?: number };

    const svg = MathJax.startup.adaptor.innerHTML(await jax.tex2svgPromise(code, { display: true }));
